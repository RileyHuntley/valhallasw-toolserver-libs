...
if time.localtime()[3] == 0:
  ...
  pagename = now.strftime("Wikipedia:Te verwijderen pagina's/Toegevoegd %Y%m%d")
  
  inhoud = "<noinclude>{{VerwijderNav}}</noinclude>\n"
  ...
  inhoud = inhoud + "=== Toegevoegd " + now.strftime("%d/%m") + ": Deel 2 ===\n\n"
  inhoud = inhoud + "=== Toegevoegd " + now.strftime("%d/%m") + ": Deel 3 ===\n"
  
  P = wikipedia.Page(wikipedia.getSite(u'nl'), pagename)
  if not P.exists():          # als 'ie bestaat doe ik lekker niks ;)
    P.put(inhoud)
    
  mainpage = wikipedia.Page(wikipedia.getSite(u'nl'), "Wikipedia:Te verwijderen pagina's")
  ...
  pagename = dan.strftime("Wikipedia:Te verwijderen pagina's/Toegevoegd %Y%m%d")
  ...
  inhoud = inhoud + "=== Toegevoegd " + dan.strftime("%d/%m") + ": Deel 2 ===\n\n"
  inhoud = inhoud + "=== Toegevoegd " + dan.strftime("%d/%m") + ": Deel 3 ===\n"
